...
const PulseBadge = ({ children, color = "emerald", className = "" }) => {
  const colorMap = {
    emerald: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/25",
    ...
    indigo: "bg-indigo-500/15 text-indigo-700 dark:text-indigo-400 border-indigo-500/25",
  }
  ...
  return (
    ...
    >
      <motion.span
        className={cn("h-1.5 w-1.5 rounded-full", dotColorMap[color] ?? dotColorMap.blue)}
...
PulseBadge.propTypes = {
  ...
  color: PropTypes.oneOf(["emerald", "amber", "red", "blue", "purple", "indigo"]),
